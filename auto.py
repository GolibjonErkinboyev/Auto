...
class Avto:
    ...
    def save_auto(self):
        ...
        with open(f"Imtihon/{last_id}.json", "w") as file:
            auto = {
                "brand": self.brand,
                "model": self.model,
            ...
            json.dump(auto, file)
    def list(self):
        dir_list = os.listdir("Imtihon/")
        for i in range(1 , len(dir_list) + 1):
            with open(f"Imtihon/{i}.json", "r") as file:
                l = json.load(file)
            print(f"N{i}{l}")
    ...
    def delete(self):
        try:
            ...
            with open(f"Imtihon/{s}.json", "r") as file:
                l = json.load(file)
            l= {}
                    
            with open(f"Imtihon/{s}.json" ,  "w") as filew:
                json.dump(l, filew)
            print(f"N{s} deleted")
        except ValueError:
...
def change():
    ...
    file_path = f'C:/Najot_talim/uy_ishi_10/Imtihon/{road}'
    with open(file_path, 'r') as file:
...
while True:
    command = input("Kommandani kiriting: /delete or /change or /exit or /list or /new: ")
